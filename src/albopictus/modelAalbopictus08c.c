...
void calculate(double *photoperiod,
  ...
  if (short_days && cold_days) { // DIAPAUSE
    ...
    //
    incubator_add(&conn10,(*egg)*(*percent_strong),0.0); // Tagged eggs
    incubator_add(&conn1,(*egg)*(1.0-(*percent_strong)),0.0); // Normal eggs
    //
  } else { // NO DIAPAUSE
    (*diap) = 0.0;
    if (!short_days && !cold_days) { // EXIT FROM DIAPAUSE
      ...
      vn0_n10 = (*n0)*param[alpha_rate_normal];
      (*n0) -= vn0_n10; // Diapausing eggs
      hatch += vn0_n10; // Eggs to hatch
    }
    // Lay normal eggs
    incubator_add(&conn1,(*egg),0.0); // Normal eggs
    //
    (*percent_strong) = 0.0;
  }
  // Develop
  ...
  (*n10) = incubator_sum(conn10);
  (*n1) = incubator_sum(conn1);
  //
